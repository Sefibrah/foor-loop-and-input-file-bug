...
export class DocumentListItemComponent implements OnDestroy {
  ...
  public readonly document = input.required<any /*FileMetadata*/>();
   ...
   */
  public readonly openPdf = output<any /*FileMetadata*/>();
  public readonly documentClicked = output<any /*FileMetadata*/>();
  ...
  protected updateFileMetadata() {
    if (
      this.form.valid &&
      (this.document().name !== this.form.value.name || this.document().description !== this.form.value.description)
    ) {
